...
| `report_activities_by_time` | گزارش زمانی: امروز، فردا، این هفته (تا آخر وقت جمعه) |
...
خروجی `edit_activity`، `report_activities_by_summary` و `show_activity_detail` به‌صورت یک کارت فعالیت رندر می‌شود (`_show_task_result` در `message_handlers.py`)، نه متن خام — همان قاعده‌ی «همه‌چیز باید فعالیت باشد».
